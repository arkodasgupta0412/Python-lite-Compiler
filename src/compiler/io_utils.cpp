...
std::string displaySymbol(const std::string& symbol) {
  ...
  if (it != pretty.end()) return it->second;
  return symbol;
}
...
void writeSVGTable(const std::string& path, const std::string& title,
  ...
  if (!out) throw std::runtime_error("Failed to write SVG file: " + path);
...
class ASTDotBuilder {
 ...
 private:
  ...
  int visitStatement(const Statement& stmt) {
    if (auto p = dynamic_cast<const AssignmentStatement*>(&stmt)) return visitAssignment(*p);
    if (auto p = dynamic_cast<const PrintStatement*>(&stmt)) return visitPrint(*p);
    if (auto p = dynamic_cast<const ForStatement*>(&stmt)) return visitFor(*p);
    if (auto p = dynamic_cast<const IfStatement*>(&stmt)) return visitIf(*p);
    if (auto p = dynamic_cast<const BlockStatement*>(&stmt)) return visitBlock(*p);
    return emitNode("UnknownStmt");
  }

  int visitExpression(const Expression& expr) {
    if (auto p = dynamic_cast<const IntLiteral*>(&expr)) return emitNode("Int: " + std::to_string(p->value));
    if (auto p = dynamic_cast<const FloatLiteral*>(&expr)) return emitNode("Float: " + std::to_string(p->value));
    if (auto p = dynamic_cast<const StringLiteral*>(&expr)) return emitNode("String: \"" + p->value + "\"");
    if (auto p = dynamic_cast<const BoolLiteral*>(&expr)) return emitNode(std::string("Bool: ") + (p->value ? "true" : "false"));
    if (auto p = dynamic_cast<const Identifier*>(&expr)) return emitNode("Id: " + p->name);
    if (auto p = dynamic_cast<const ListLiteral*>(&expr)) return visitList(*p);
    if (auto p = dynamic_cast<const RangeExpression*>(&expr)) return visitRange(*p);
    if (auto p = dynamic_cast<const UnaryExpression*>(&expr)) return visitUnary(*p);
    if (auto p = dynamic_cast<const BinaryExpression*>(&expr)) return visitBinary(*p);
  ...
  int visitRange(const RangeExpression& expr) {
    ...
    if (expr.step) emitEdge(root, visitExpression(*expr.step));
    return root;
  }
};
...
std::string ProgramReader::read(const std::string& path) const {
  ...
  if (!in) throw std::runtime_error("Failed to open source file: " + path);
...
bool OutputWriter::extractLineColumn(const std::string& message, int& line, int& column) {
  ...
  std::smatch match;
  if (!std::regex_search(message, match, rgx) || match.size() < 3) return false;
...
void OutputWriter::writeDiagnosticWithSnippet(const std::string& kind,
  ...
  std::cerr << "    | ";
  for (int i = 1; i < column; ++i) std::cerr << ' ';
  std::cerr << "^\n";
}
...
void OutputWriter::writeParseTableReport(const std::string& path, const GrammarArtifacts& artifacts) const {
  ...
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Failed to write report file: " + path);
...
void OutputWriter::writeGrammarImages(const std::string& outputDir, const GrammarArtifacts& artifacts) const {
  ...
  for (const auto& [_, row] : artifacts.parseTable) {
    for (const auto& [terminal, __] : row) terminals.insert(terminal);
  }
